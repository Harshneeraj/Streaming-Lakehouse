"""
Ad-hoc test: same record key, different partition path  (queried via Trino).

Scenario:
  1) INSERT  (id=A, part=P, ts=1)
  2) UPSERT  (id=A, part=S, ts=2)   <- same key, NEW partition

Writes two Hudi COW tables into the `lake` DB and syncs them to the Hive
Metastore so Trino can query them:
  * lake.pk_move_true   (hoodie...update.partition.path = true)
  * lake.pk_move_false  (hoodie...update.partition.path = false)
"""
...
def run_case(spark, table_name, base_path, update_partition_path):
    ...
    df2.write.format("org.apache.hudi") \
        .options(**common_opts(table_name, base_path, update_partition_path)) \
        .mode("append").save(base_path)
    print(f"[{table_name}] written + synced")


def main():
    spark = SparkSession.builder.appName("test_partition_change").enableHiveSupport().getOrCreate()
    spark.sparkContext.setLogLevel("ERROR")
    spark.sql("CREATE DATABASE IF NOT EXISTS lake")

    run_case(spark, "pk_move_true",  "s3a://lakehouse/_test/pk_move_true",  "true")
    run_case(spark, "pk_move_false", "s3a://lakehouse/_test/pk_move_false", "false")
